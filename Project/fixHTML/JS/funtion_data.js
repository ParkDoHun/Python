...
function next_Page(tid, count, number) {
    ...
    location.href = "http://127.0.0.1:5500/html"+'/'+number+'~'+".html?"+number;
};
...
$(function dataSend(){
 ...
 /*다음 페이지로 이동과 동시에 Session에 값 추가*/
function end_Page(tid, count, number) {
    ...
    var survayValue3 = document.getElementsByName("survay3");
    if (question_cnt == 0 ){
        var survayValue4 = document.getElementsByName("survay3");
    }
    // 두 개의 값이 체크되어 있는지 검사
    var chk_cnt = 0;
    ...
    //값 저장 후 페이지 이동
    localStorage.setItem("allEntries", JSON.stringify(existingEntries));
    location.href = "http://127.0.0.1:5500/html/"+'end'+".html?"+number;
};